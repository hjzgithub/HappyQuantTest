...
def get_model_train_and_test(handler: Handler, model_id, X, y):
...
def run_model(handler: Handler, model_name, model_id, *args):
...
def rolling_run_models(model_name, 
                       model_id, 
                       df_factors, 
                       df_tags, 
                       back_window, 
    ...
    if use_parallel:
        list_preds = joblib.Parallel(n_jobs=-1, backend='loky', verbose=0)( \
                        joblib.delayed(run_model)(handler, model_name, model_id, df_factors, df_tags, i, \
        ...
        list_preds = [run_model(handler, model_name, model_id, df_factors, df_tags, i, \
                                back_window, target_type, standardize_method, single_test_method, combine_method, multi_test_method)\
                      for i in range(back_window+1, len(df_factors.index))]
