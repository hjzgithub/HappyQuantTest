...
def get_data_split(df_factors, df_tags, i, back_window, with_pca):
    ...
    if with_pca:
        #X = fixed_pca(X)
        X = fixed_pca(ts_z_score(pd.DataFrame(X)))
...
def rolling_run_models(model_name, model_id, df_factors, df_tags, back_window, with_pca=False, use_parallel=True) -> pd.DataFrame:
    ...
    if use_parallel:
        list_preds = joblib.Parallel(n_jobs=-1, backend='loky', verbose=0)( \
                        joblib.delayed(run_model)(handler, model_name, model_id, df_factors, df_tags, i, back_window, with_pca)\
                        for i in range(back_window+1, len(df_factors.index)))
    else:
        list_preds = [run_model(handler, model_name, model_id, df_factors, df_tags, i, back_window, with_pca)\
                      for i in range(back_window+1, len(df_factors.index))]
